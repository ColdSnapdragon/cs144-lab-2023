...
void TCPReceiver::receive( TCPSenderMessage message, Reassembler& reassembler, Writer& inbound_stream )
{
  ...
  // 除了SYN自身之外的报文要考虑SYN占一个序列号
  if ( message.SYN ) {
    _isn = Wrap32 { message.seqno };
    _has_isn = true;
  }
  uint64_t index = message.seqno.unwrap( _isn + ( !message.SYN ), inbound_stream.bytes_pushed() );
...
TCPReceiverMessage TCPReceiver::send( const Writer& inbound_stream ) const
{
  ...
  Wrap32 ackno = Wrap32::wrap( index, _isn + 1 ) + inbound_stream.is_closed(); // FIN占一个序列号(无形地出现在末尾)
